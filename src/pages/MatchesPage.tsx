...
export function MatchesPage() {
  ...
  const statusLabels: Record<MatchFilter, string> = {
    all: t('matches.all', language),
    live: t('matches.live', language),
    upcoming: t('matches.upcoming', language),
    finished: t('matches.finished', language),
  };
  ...
  if (error) return <div className="glass-card p-12 text-center"><X size={40} className="mx-auto mb-4 text-wc-red/50" /><h3 className="text-lg font-bold text-text mb-2">{t('matches.failedLoad', language)}</h3><p className="text-text-secondary text-sm">{t('matches.failedDetail', language)}</p></div>;
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
        <h1 className="text-2xl sm:text-3xl font-bold gradient-text">{t('matches.title', language)}</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder={t('matches.searchPlaceholder', language)} value={search} onChange={(e) => setSearch(e.target.value)}
        ...
        <button onClick={() => setShowFilters(!showFilters)}
          ...
          <Filter size={16} /> {t('matches.filters', language)} {filtersActive && <span className="w-2 h-2 rounded-full bg-white" />}
        </button>
      </div>
      ...
      {showFilters && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="glass-card p-4">
          <div className="flex flex-wrap gap-4">
            <div>
              <label className="text-xs font-bold uppercase tracking-wider text-text-secondary block mb-2">{t('matches.status', language)}</label>
            ...
            <div>
              <label className="text-xs font-bold uppercase tracking-wider text-text-secondary block mb-2">{t('matches.group', language)}</label>
              <div className="flex gap-1.5 flex-wrap">
                <button onClick={() => setGroupFilter('all')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${groupFilter === 'all' ? 'bg-wc-blue text-white' : 'bg-card border border-border text-text-secondary hover:bg-wc-blue/5'}`}>{t('matches.all', language)}</button>
                {GROUPS.map((g) => (
                  <button key={g} onClick={() => setGroupFilter(g)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${groupFilter === g ? 'bg-wc-blue text-white' : 'bg-card border border-border text-text-secondary hover:bg-wc-blue/5'}`}>{tf('common.groupLabel', language, g)}</button>
                ))}
              </div>
            </div>
          </div>
      ...
      <p className="text-sm text-text-secondary">{tf('matches.showingOf', language, filtered.length, matches?.length ?? 0)}</p>

      {filtered.length === 0 ? (
        <div className="glass-card p-12 text-center"><Calendar size={40} className="mx-auto mb-4 text-text-secondary/30" /><p className="text-text-secondary font-medium">{t('matches.noMatchesFound', language)}</p></div>
      ) : (
              ...
              <div key={match.id} className="glass-card-hover p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-col items-center mx-3">
                    ...
                    {match.finished && <span className="text-[9px] text-text-secondary/40 mt-0.5">{t('matches.fullTime', language)}</span>}
                  </div>
                ...
                <div className="flex items-center justify-center gap-4 mt-3 pt-3 border-t border-border">
                  <span className="text-[10px] text-text-secondary/50">{tf('common.matchLabel', language, match.id)}</span>
                  <span className="text-[10px] text-text-secondary/50">{match.type?.replace('_', ' ') ?? t('matches.groupStage', language)}</span>
                </div>
              </div>
            );
          })}
