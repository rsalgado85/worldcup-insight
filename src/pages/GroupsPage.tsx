    ...
    <div className="glass-card-hover p-0 overflow-hidden">
      <div className="wc-gradient p-4 text-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2"><Trophy size={18} className="text-wc-gold" /><h3 className="text-lg font-black">Group {group.name}</h3></div>
          <span className="text-xs font-semibold text-white/70">{teams.length} teams</span>
        </div>
      </div>
      <div className="p-3">
        {teams.length === 0 ? <p className="text-sm text-text-secondary text-center py-4">No teams</p> : (
      ...
      <div className="px-4 py-2.5 bg-bg/50 border-t border-border flex items-center justify-between text-[10px] text-text-secondary">
        <span>Total goals: {teams.reduce((s, t) => s + t.gf, 0)}</span>
        <span>Avg pts: {teams.length ? (teams.reduce((s, t) => s + t.pts, 0) / teams.length).toFixed(1) : '—'}</span>
      </div>
    </div>
  );
}
...
export function GroupsPage() {
  ...
  if (error) return <div className="glass-card p-12 text-center"><Users size={40} className="mx-auto mb-4 text-text-secondary/30" /><h3 className="text-lg font-bold text-text mb-2">Failed to load groups</h3><p className="text-text-secondary text-sm">Please try again later.</p></div>;

  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
        <h1 className="text-2xl sm:text-3xl font-bold gradient-text">Groups</h1>
        <p className="text-sm text-text-secondary">All 12 groups of the FIFA World Cup 2026™ — 48 teams competing for the trophy</p>
      </motion.div>

      {!groups || groups.length === 0 ? (
        <div className="glass-card p-12 text-center"><Users size={40} className="mx-auto mb-4 text-text-secondary/30" /><p className="text-text-secondary font-medium">No group data available</p></div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {groups.map((group: Group) => <GroupCard key={group.name} group={group} />)}
        </div>
      )}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="wc-gradient rounded-2xl p-6 text-white text-center">
        ...
        <h3 className="text-lg font-black mb-2">Road to the Final</h3>
        <p className="text-sm text-white/70 max-w-2xl mx-auto">The top 2 teams from each group advance to the Round of 32. The 8 best third-placed teams also qualify. Then it's knockout football all the way to the final on July 19, 2026.</p>
      </motion.div>
    </div>
  );
}
